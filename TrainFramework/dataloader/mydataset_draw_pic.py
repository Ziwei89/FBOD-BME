    ...
    for i in range(heatmap_shape[0]): # height
        for j in range(heatmap_shape[1]): # weight
            ...
            if heatmap[i][j] > 0:
                ...
                cv2.circle(img, (int(ref_point_position[0]), int(ref_point_position[1])), 1, (255,0,0), -1, cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    input_mode = "RGB"
    continues_num=5
    default_inner_proportion=1.0
    assign_method="binary_assign"
    dataset_image_path = "../../../FBOD-BSPL_old/dataset/val/images/"
    ...
    train_data = CustomDataset(data, image_size, image_path=dataset_image_path, input_mode=input_mode, continues_num=continues_num,
                               stride=stride, default_inner_proportion=default_inner_proportion, assign_method=assign_method)
    ...
    for i, item in enumerate(dataloader):
        ...
        for bs in range(batch_size):
            ...
            test_img2=copy.deepcopy(test_img)
                ...
                if i == int(continues_num/2):
                    if len(label_points) != 0:
                        for box in label_points:
                            ...
                            cv2.rectangle(test_img2,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(255,255,0),2)
                        cv2.imwrite("./test_output_img/label_test{}.jpg".format(i), image)
                        cv2.imwrite("./test_output_img/cls_loc_test{}.jpg".format(bs), test_img)
                        cv2.imwrite("./test_output_img/loc_test{}.jpg".format(bs), test_img2)
                    else:
                        cv2.imwrite("./test_output_img/label_test{}.jpg".format(i), image)
                else:
                    cv2.imwrite("./test_output_img/label_test{}.jpg".format(i), image)
